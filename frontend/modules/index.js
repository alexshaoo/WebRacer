...
const countItDown = function () {
	...
	document.getElementById("WebRacer").innerHTML += s[s.length - countdown--];
};

const timer = window.setInterval(countItDown, 200);
...
script.src = "https://ricostacruz.com/jquery.transit/jquery.transit.min.js";
...
background.initializr = function () {
	...
	$this.style = {bubbles_color: "lightslategray", stroke_width: 0, stroke_color: "black"};
...
background.generate_bubbles = function () {
	...
	bolla.transit(
		{top: $this.rn($this.wh / 2, $this.wh / 2 - 60), transform: "scale(" + rn_size + ") rotate(" + $this.rn(-360, 360) + "deg)", opacity: 0},
